...
- **Итоги:** В рамках проекта решена задача регрессии — предсказание средней зарплаты по описанию вакансии с hh.ru. Финальная модель — LightGBM с гиперпараметрами, подобранными через Optuna — достигла **MAE = 21 785 руб.** на тестовой выборке при R2 = 0.582. По сравнению с baseline (DummyRegressor, MAE = 43 035 руб.) ошибка снижена почти вдвое, что подтверждает практическую ценность обученной модели. Модель развёрнута в виде полноценного сервиса: FastAPI-бэкенд принимает данные вакансии и возвращает предсказание; Streamlit-интерфейс поддерживает как ручной ввод, так и загрузку карточки напрямую с hh.ru через официальный API. Оба сервиса упакованы в Docker Compose и работают в связке.

- **Ограничения:** Модель систематически недооценивает высокие зарплаты — это видно как по графику остатков, так и по относительно скромному R2. Причина — малое число таких вакансий в обучающей выборке: правый хвост распределения обрезался при удалении выбросов, и модель плохо интерполирует туда. Спектральная кластеризация по названиям вакансий не принесла улучшения качества, хотя сам пайплайн технически корректен — сигнал из коротких текстовых названий оказался недостаточно богатым для дополнительной дискриминации. CatBoost через нативный text_features API показал себя хуже LightGBM на коротких текстах, что ограничило возможности ансамблирования.

- **Возможные улучшения:** Наиболее перспективным направлением выглядит замена TF-IDF на эмбеддинги предобученной языковой модели (например, ruBERT) — это позволит лучше извлекать семантику из описаний вакансий и, вероятно, существенно снизит ошибку в сегменте нестандартных и высокооплачиваемых позиций. Также стоит рассмотреть отдельное моделирование хвоста распределения (например, через пороговое разбиение или квантильную регрессию), добавление признаков даты публикации для учёта сезонности рынка труда, а также расширение датасета за счёт более длительного сбора через API hh.ru с охватом разных регионов и профессиональных областей
